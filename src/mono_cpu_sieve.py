    ...
    if len(block) > 512:
        ...
        for z in range(len(coeff)):
            ...
            for i in range(coeff[z][2]):
                ...
                relations, smooth_number, partial_relations, possible_smooth, full_found, partial_found, graph, size_partials, parent = handle_possible_smooth(value, tmp_smooth, full_found, partial_found, relations,
                                                                                                                                                               smooth_number, partial_relations, possible_smooth, graph,
                                                                                                                                                               size_partials, parent, cycle_len, n)
...
def find_relations(primes, const, prod_primes, bounds, target, logs, a, b, flag_use_batch_smooth_test, n, LOG_PATH):
    ...
    while len(relations) <= len(primes)+10:
        ...
        if flag_use_batch_smooth_test:
            ...
            relations, smooth_number, full_found, partial_relations, partial_found, possible_smooth, graph, size_partials, parent = sieve_and_smooth(relations, smooth_number, full_found, partial_relations, partial_found, possible_smooth,
                                                                                                                                                     graph, a, b, poly_selected, logs, primes, param, sieve_len, skipped, prime_start,
                                                                                                                                                     const_1, const_2, tmp1, tmp2, tmp3, size_partials, parent, cycle_len, n)
